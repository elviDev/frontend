...
interface EnhancedAvatarProps {
  user: {
    ...
    name: string;
    avatar?: string | null;
    avatar_url?: string | null; // Support both naming conventions
    role?: string;
    isOnline?: boolean;
  };
  size?: 'xs' | 'sm' | 'md' | 'lg' | 'xl';
...
export const EnhancedAvatar: React.FC<EnhancedAvatarProps> = ({
  ...
  // Get live avatar URL (prioritizes Redux + local storage for current user)
  const providedAvatarUrl = user.avatar || user.avatar_url;
  const { avatarUrl, isCurrentUser, isLoading } = useUserAvatar(user.id, providedAvatarUrl);
  ...
  const AvatarContent = () => (
    <View style={{ position: 'relative' }}>
      {avatarUrl && (avatarUrl.includes('http') || avatarUrl.includes('https') || avatarUrl.includes('file://')) ? (
        <View style={{ position: 'relative' }}>
          <Image
          ...
          />
          {/* Show loading indicator for current user when updating */}
...
// Re-export as Avatar for backward compatibility
export const Avatar = EnhancedAvatar;